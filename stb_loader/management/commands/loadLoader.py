...
class Command(BaseCommand):
    def main(self, dtime):
        self.stdout.write(
            self.style.SUCCESS(f'{dtime}: Load data shift states Loader')
        )
        ...
        shift_states_df = pd.DataFrame(columns=['Time Start','Time End','Equipment','Reason'])
        ...
        shift_states_df['Time Start'] = pd.to_datetime(shift_states_df['Time Start'], format="%Y-%m-%d %H:%M:%S",utc=True)
        shift_states_df['Time End'] = pd.to_datetime(shift_states_df['Time End'], format="%Y-%m-%d %H:%M:%S",utc=True)
        # fill null reason
        ...
        self.stdout.write(
            self.style.SUCCESS(f'{dtime}: Load data shift activity')
        )
        ...
        data = cursor_jigsaw.fetchall()
        shift_activity_df = pd.DataFrame(columns=['Time Start','Time End','Equipment','Shovel','Activity'])
        i = 0
        for d in data:
            ...
            i+=1
        shift_activity_df['Time Start'] = pd.to_datetime(shift_activity_df['Time Start'], format="%Y-%m-%d %H:%M:%S",utc=True)
        shift_activity_df['Time End'] = pd.to_datetime(shift_activity_df['Time End'], format="%Y-%m-%d %H:%M:%S",utc=True)

        cursor_jigsaw.close()

        self.stdout.write(
            self.style.SUCCESS(f'{dtime}: Load data MCR BD Loader')
        )
        ...
        breakdown_df = pd.DataFrame(columns=["Time Start",'Time End','Equipment','bd_code','remarks','Standby Code'])
        for i, d in enumerate(data):
            a = str(f"{d[6]} {d[7]}")
            if d[8] != "":
                b = str(f"{d[8]} {d[9]}")
        ...
        breakdown_df['Time Start'] = pd.to_datetime(breakdown_df['Time Start'], format="%Y-%m-%d %H:%M",utc=True)
        breakdown_df['Time End'] = pd.to_datetime(breakdown_df['Time End'],format="%Y-%m-%d %H:%M",utc=True)
        cursor_mcr.close()  
        ...
        breakdown_df['statusBDC'] = breakdown_df['bd_code'].combine_first(breakdown_df['statusBDC'])
        breakdown_df['BD Status'] = breakdown_df['remarks'].apply(f.BDstatus)
        breakdown_df['remarks'] = breakdown_df[['remarks', 'BD Status', 'statusBDC']].apply(lambda x: ';'.join(x.dropna()), axis=1)
        ...
        result = pd.DataFrame(columns=['Time Start','Time End','Equipment','Standby Code','remarks'])
        ...
        for loader in list_loader:
            ...
            if loader_bd.empty or (loader_bd['durasi'] < 60).any():
                ss = loader_df[loader_df['Equipment'] == loader].sort_values('Time Start').reset_index(drop=True).copy()
                ...
                data.loc[data['Standby Code'].astype(str).str.contains('SS', case=True),['Standby Code','Rank']] = ['WH',20]
                ...
                if isFriday and hour == 12:
                    ...
                    group = (data['Standby Code']!=data['Standby Code'].shift()).cumsum()
                ...
                if isFriday and hour == 11:
                    data = f.split30min(data)
                    conditions = ((data["Time Start"] >= half_time) & (data['Rank']>5)) | ((data["Time Start"] >= half_time) & (data['Standby Code']=='S8'))
                ...
                result = pd.concat([result, data[['Time Start','Time End','Equipment','Standby Code','remarks']]], axis=0)
                result = result[result['Time Start'] != result['Time End']].reset_index(drop=True)
            else:
                if hour == 6:
                    loader_bd = f.split30min(loader_bd)
                result = pd.concat([result, loader_bd[['Time Start','Time End','Equipment','Standby Code','remarks']]], axis=0)
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            self.style.SUCCESS(f'{dtime}: Load data shift states, shift activity, dan MCR BD Loader')
        )

        data = self.main(dtime)
        for i, row in data.iterrows():
            unit,_ = loaderID.objects.get_or_create(
                unit=row['Equipment']
            )
        ...
        self.stdout.write(
            self.style.SUCCESS(f'{dtime}: Load data sukses')
        )