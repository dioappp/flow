...
def generate_s12(loader:str, act_df: DataFrame, ts: Timestamp, te: Timestamp ) -> DataFrame:
    trucks_act= act_df[act_df['Shovel']==loader].sort_values(by=['Equipment', 'Time Start']).reset_index(drop=True)

    trucks_act = trucks_act[trucks_act['Time Start']!=trucks_act['Time End']].copy()
    
    trucks_act['groups'] = (trucks_act['Time Start']!=trucks_act['Time End'].shift()).cumsum()
    ...
    RUN = True
    while RUN:
        eval = ~((trucks_act['Time Start'] > trucks_act['Time Start'].shift()) & (trucks_act['Time End'] < trucks_act['Time End'].shift()))
        RUN = not all(eval)
...
def fill_gap(trucks_act: DataFrame, ts: Timestamp, te: Timestamp, loader: str) -> DataFrame:
...
def summarize(loader:str, s12: DataFrame, ss: DataFrame) -> DataFrame:
    ...
    for _, row_s12, in s12.iterrows(): 
        ...
        overlaps_df = ss[(ss['Time Start'] < row_s12['Time End']) & (ss['Time End'] > row_s12['Time Start']) & (ss['Rank'] >= row_s12["Rank"])]
    ...
    output_df = pd.DataFrame(result_rows).sort_values('Time Start').reset_index(drop=True) 
...
def split(s12: DataFrame, ss: DataFrame) -> DataFrame:
    ...
    for _, row_ss in ss.iterrows():
        for _, row_s12 in s12.iterrows():
            if (row_s12['Time Start'] < row_ss['Time End']) and (row_s12['Time End'] > row_ss['Time Start']):
                start_time = max(row_s12['Time Start'],row_ss['Time Start'])
                end_time = min(row_s12['Time End'],row_ss['Time End'])

                new_row = pd.DataFrame({'Time Start': [start_time], 'Time End': [end_time]})
...
def combine_bd(ss: DataFrame, bd: DataFrame) -> DataFrame:
    ...
    for _, row in data.iterrows():
        ...
        if (row['Time Start'] < result.loc[l-1, 'Time End']) and (row['Time End'] > result.loc[l-1, 'Time End']):
            result.loc[l] = row
...
def adjustTimeEnd(data: DataFrame) -> DataFrame:
    ...
    len_data = len(data)
    if len_data > 1:
        data.loc[0:len_data-2,'Time End'] = data.loc[1:len_data, 'Time Start'].values
...
def statusBDC(remark:str)->str:
    C1 = ["eng", "ps", "low power", "service", "err", "e 0", "e-", "mati mendadak", "fip", "radiator", "start", "overh", "v-belt", "van belt", "midlife", "mid life"]
    C2 = ["tran"]
    C3 = ["spro", "final", "trac", "undercar"]
    C4 = ["tyre", "tire", "suspensi", "r1", "r2", "r3", "r4"]
    C5 = ["ster", "drol", "motor", "hyd", "dum", "cyl", "bo", "hose cyl", "cil", "hose bo"]
...
def BDstatus(remark:str) -> str:
    B5 = ['acc', 'den', 'insi']
    BS = ['g o h', 'goh', 'overhoul', 'back', 'serv', 'mid', 'ps', 'sch', 'ovh', 'o v h']
