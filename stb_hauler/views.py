...
def index(request):
    ...
    return render(request,'stb_hauler/index.html',{'jam':loop_times, 'stb':stb, 'ritase':tipe_ritase})

def reportDataSTB(request):
    date_pattern = request.POST.get('date') 
    hour_pattern = request.POST.get('hour')

    maindata = HaulerStatus.objects.filter(date=date_pattern, hour=hour_pattern)
...
def timeline(request):
    ...
    maindata = HaulerStatus.objects.filter(date=date_pattern, hour=hour_pattern, unit__jigsaw=unit_pattern).order_by('timeStart')
    ...
    for i, d in enumerate(maindata):
        ...
        x['timeEnd'] = (maindata[0]['timeStart'] + timedelta(hours=1)).strftime('%Y-%m-%d %H:%M:%S') if i == len(maindata) - 1 else maindata[i+1]['timeStart'].strftime('%Y-%m-%d %H:%M:%S')

        x['label'] = d['standby_code']
        response['state'].append(x)

    ritasedata = ritase.objects.filter(date=date_pattern, hour=hour_pattern, truck_id__jigsaw=unit_pattern).order_by('time_full')
