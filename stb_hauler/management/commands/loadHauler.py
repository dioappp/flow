...
class Command(BaseCommand):
    def main(self,dtime):
        self.stdout.write(
            self.style.SUCCESS(f'{dtime}: Load data shift states Hauler')
        )
        ...
        self.stdout.write(
            self.style.SUCCESS(f'{dtime}: Load data MCR BD Hauler')
        )
        # Data Breakdown
        bd_sql = """
        ...
        for hauler in list_hauler:
            ...
            if hauler_bd.empty or (hauler_bd['durasi'] < 60).any():
                data = hauler_df[hauler_df['Equipment'] == hauler].sort_values('Time Start').reset_index(drop=True).copy()
                ...
                result = pd.concat([result, hauler_bd[['Time Start','Time End','Equipment','Standby Code','remarks']]], axis=0)
    ...
    def handle(self, *args, **options):
        ...
        for i, row in data.iterrows():
            unit,_ = truckID.objects.get_or_create(
                jigsaw=row['Equipment']
            )
