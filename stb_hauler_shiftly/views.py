...
def index(request):
    ...
    return render(request,'stb_hauler_shiftly/index.html',{'stb':stb, 'ritase':tipe_ritase})

def reportDataSTB(request):
    date_pattern = request.POST.get('date') 
    shift_pattern = int(request.POST.get('shift'))

    maindata = HaulerStatus.objects.filter(report_date=date_pattern, shift=shift_pattern)
    ...
    for d in page_obj:
        ...
        x['action'] = '<div id="data-' + str(d['unit__jigsaw']) + '" data-id="' + str(d['unit__jigsaw']) + '" class="d3-timeline"></div>'
...
def timeline(request):
    ...
    maindata = HaulerStatus.objects.filter(report_date=date_pattern, shift=shift_pattern, unit__jigsaw=unit_pattern).order_by('timeStart')
    ...
    ritasedata = ritase.objects.filter(date=date_pattern, shift=shift_pattern, truck_id__jigsaw=unit_pattern).order_by('time_full')
    ritasedata = ritasedata.values('id', 'time_full','time_empty','type','loader_id__unit','dump_location','truck_id__OB_capacity')
