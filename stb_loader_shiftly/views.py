...
def index(request):
    ...
    stb = ["BS","BUS","S8","S18","S5A","S5B","S5C","S6","S1","S2","S10A","S10B","S15","S19","S12","S13","S14A", "S14B","S14C","S17","S4","S11", "S20C", "WH", "WH general"]
    tipe_ritase = ["OB","General","Coal","Top Soil","IPD","Mud","Spoil"]
    return render(request, 'stb_loader_shiftly/index.html',{'jam':loop_times, 'stb':stb, 'ritase':tipe_ritase})

def reportDataSTB(request):
    date_pattern = request.POST.get('date') 
    shift_pattern = int(request.POST.get('shift'))

    maindata = LoaderStatus.objects.filter(report_date=date_pattern, shift=shift_pattern)
    ...
    for d in page_obj:
        x = {}
        x['unit'] = d['unit__unit']
        x['action'] = '<div id="data-' + str(d['unit__unit']) + '" data-id="' + str(d['unit__unit']) + '" class="d3-timeline"></div>'
...
def timeline(request):
    ...
    maindata = LoaderStatus.objects.filter(report_date=date_pattern, shift=shift_pattern, unit__unit=unit_pattern).order_by('timeStart')
    ...
    for h in hour_list:
        ...
        for i, d in enumerate(data):
            ...
            if not h == 6:
                x['timeEnd'] = (data[0]['timeStart'] + timedelta(hours=1)).strftime('%Y-%m-%d %H:%M:%S') if i == len(data) - 1 else data[i+1]['timeStart'].strftime('%Y-%m-%d %H:%M:%S')
            else:
                x['timeEnd'] = (data[0]['timeStart'] + timedelta(minutes=30)).strftime('%Y-%m-%d %H:%M:%S') if i == len(data) - 1 else data[i+1]['timeStart'].strftime('%Y-%m-%d %H:%M:%S')
            if not show_hanging:
    ...
    # ritasedata = ritase.objects.filter(date=date_pattern, hour=hour_pattern, loader_id__unit=unit_pattern).order_by('time_full')
    # ritasedata = ritasedata.values('id', 'time_full','time_empty','type','truck_id__jigsaw','dump_location','truck_id__OB_capacity')

    # for d in ritasedata:
    #     x = {}
    #     x['database_id'] = d['id']
    #     x['time_full'] = d['time_full'].strftime('%Y-%m-%d %H:%M:%S')
    #     x['time_empty'] = d['time_empty'].strftime('%Y-%m-%d %H:%M:%S') if d['time_empty'] else 'N/A'
    #     x['type'] = d['type']
...
def export_excel(request):
    ...
    result[['remarks', 'BD Status', 'statusBDC']] = result['remarks'].str.split(';', expand=True)
    result['Location'] = 'NT/CT (not yet)'
    result['hour'] = result['hour'].apply(lambda x: f"{x:02d}:00 - {(x+1):02d}:00" if ((0 <= x < 6) or (6 < x <= 23)) else "06:30 - 07:00" if shift == 1 else "06:00 - 06:30")
    column_order = ['report_date', 'Project', 'Location', 'unit', 'date','hour', 'BD Status', 'statusBDC', 'standby_code', 'durasi', 'remarks']
